...
/// 流式中途传输失败且已向用户输出过内容时的签名错误。此时端点故障转移
/// 被抑制(避免输出到一半换供应商产生重复/拼接内容),整轮回复因此中断。
/// 这是供应商/网络层的瞬态故障,而非协议或请求构造错误。命中后调用方应
/// 以友好提示收束本轮并走"中断回合续传"恢复路径,而不是把裸错误丢给用户。
pub fn is_stream_failed_after_emitting_output(error: &anyhow::Error) -> bool {
    let text = format!("{error:#}");
    text.contains("LLM stream failed after emitting output")
}
...
mod overflow_classifier_tests {
    ...
    #[test]
    fn stream_failed_after_output_classifier() {
        use super::is_stream_failed_after_emitting_output;
        let hit = anyhow::anyhow!("boom")
            .context("LLM stream failed after emitting output; endpoint failover was suppressed");
        assert!(is_stream_failed_after_emitting_output(&hit));
        let miss = anyhow::anyhow!("boom")
            .context("LLM request was rejected; endpoint failover was suppressed");
        assert!(!is_stream_failed_after_emitting_output(&miss));
    }
}
